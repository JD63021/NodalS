...
// P5E_CURRENT_MAIN_PTAP_COMPAT
// Keep the historical PtAP API used by the current PMIS/CF reference path.
// The SA hierarchy itself routes through build_sa_ptap_dispatch().
inline CSRHost build_sa_ptap(
    const CSRHost&A,const SATransferHost&P,int nextLevel)
{
  return build_sa_ptap_reference(A,P,nextLevel);
}
