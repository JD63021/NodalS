...
inline H2FineCSRHost build_h2_fine_csr_host_reference(const G4SetupHost&S){
  using gate5_clock=std::chrono::steady_clock;
  const auto gate5_all0=gate5_clock::now();
  H2FineCSRHost H;
  ...
  const auto gate5_count0=gate5_clock::now();
  ...
  const auto gate5_count1=gate5_clock::now();
  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=incidence_count seconds=%.6f freeVel=%d status=PASS\n",
              std::chrono::duration<double>(gate5_count1-gate5_count0).count(),H.nv);
  const auto gate5_scan0=gate5_clock::now();
  ...
  const auto gate5_scan1=gate5_clock::now();
  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=incidence_scan_alloc seconds=%.6f entries=%lld status=PASS\n",
              std::chrono::duration<double>(gate5_scan1-gate5_scan0).count(),
              (long long)nt);
  const auto gate5_fill0=gate5_clock::now();
  ...
  const auto gate5_fill1=gate5_clock::now();
  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=incidence_fill seconds=%.6f entries=%zu status=PASS\n",
              std::chrono::duration<double>(gate5_fill1-gate5_fill0).count(),
              H.packed.size());
  std::vector<std::int32_t>().swap(cur);
  std::vector<std::int32_t>().swap(count);

  const auto gate5_init0=gate5_clock::now();
  ...
  const auto gate5_init1=gate5_clock::now();
  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=output_init_reserve seconds=%.6f status=PASS\n",
              std::chrono::duration<double>(gate5_init1-gate5_init0).count());

  const auto gate5_rows0=gate5_clock::now();
  std::uint64_t gate5_targetTotal=0;
  std::uint32_t gate5_targetMax=0;
  for(int c=0;c<H.n;++c){
    target.clear();
    ...
    gate5_targetTotal+=(std::uint64_t)target.size();
    gate5_targetMax=std::max<std::uint32_t>(gate5_targetMax,(std::uint32_t)target.size());
  ...
  const auto gate5_rows1=gate5_clock::now();
  H.directedContrib=H.slot.size();
  H.rowMean=H.n?(double)H.col.size()/(double)H.n:0.0;
  const auto gate5_all1=gate5_clock::now();

  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=row_symbolic_slot seconds=%.6f rows=%d nnz=%zu directed=%llu targetTotal=%llu targetMax=%u rowMax=%u rowMean=%.6f directedPerNnz=%.6f status=PASS\n",
              std::chrono::duration<double>(gate5_rows1-gate5_rows0).count(),
              H.n,H.col.size(),
              (unsigned long long)H.directedContrib,
              (unsigned long long)gate5_targetTotal,
              gate5_targetMax,H.rowMax,H.rowMean,
              H.col.empty()?0.0:(double)H.directedContrib/(double)H.col.size());
  std::printf("NODALS_GPU_GATE5_FINECSR_PROFILE stage=total seconds=%.6f csrMiB=%.3f metadataMiB=%.3f totalMiB=%.3f status=PASS\n",
              std::chrono::duration<double>(gate5_all1-gate5_all0).count(),
...
// -----------------------------------------------------------------------------
// Gate 5A: parallel fine-pressure CSR symbolic + uint8 row-slot construction.
//
// The exact velocity -> (pressure cell, local basis) incidence remains the same.
// Pressure rows are independent.  To avoid O(nRows*maxWidth) scratch, use:
//
//   pass 1: rebuild row target list, sort/unique, record exact nnz and directed
//           contribution counts.
//   prefix: exact final row/contrib offsets and allocations.
//   pass 2: rebuild the same target list, sort/unique, write final col/diag/slot.
//
// Per-row target order and sorted-unique columns are identical to the reference.
// -----------------------------------------------------------------------------

inline int gate5a_threads(){
  int n=0;
...
inline void gate5a_build_target(
...
inline H2FineCSRHost build_h2_fine_csr_host_parallel(const G4SetupHost&S){
  ...
  const int nth=gate5a_threads();

  // Exact reference incidence construction; this is only ~0.075 s at 2M.
  ...
  std::printf(
    "NODALS_GPU_GATE5A_FINECSR_STAGE stage=incidence status=PASS entries=%zu seconds=%.6f threads=%d rssMiB=%.3f hwmMiB=%.3f\n",
    H.packed.size(),std::chrono::duration<double>(inc1-inc0).count(),nth,
    g4a_status_mib("VmRSS"),g4a_status_mib("VmHWM"));
  std::fflush(stdout);

  // Pass 1: row counts only.
  ...
  g4a_parallel_chunks(H.n,nth,128,[&](int t,int cb,int ce){
    ...
    for(int c=cb;c<ce;++c){
      gate5a_build_target(S,H,c,target);
      gate5a_make_uniq(target,uniq);

      if(uniq.empty())
        throw std::runtime_error("H2 empty pressure CSR row");
      if(uniq.size()>255)
        throw std::runtime_error("H2 row width exceeds uint8 slot capacity");
    ...
    targetMaxThread[(std::size_t)t]=
      std::max(targetMaxThread[(std::size_t)t],tmax);
    rowMaxThread[(std::size_t)t]=
      std::max(rowMaxThread[(std::size_t)t],rmax);
  });
  const auto p11=clock::now();

  // Prefix exact final slices.
  ...
  // Pass 2: exact final columns, diagonal positions, and row-local slot map.
  ...
  g4a_parallel_chunks(H.n,nth,128,[&](int,int cb,int ce){
    ...
    for(int c=cb;c<ce;++c){
      ...
      if((std::int32_t)uniq.size()!=rowCount[(std::size_t)c] ||
        ...
        throw std::runtime_error("Gate5A pass mismatch");

      const std::int32_t rb=H.row[(std::size_t)c];
      std::copy(uniq.begin(),uniq.end(),H.col.begin()+rb);

      auto dit=std::lower_bound(uniq.begin(),uniq.end(),c);
      H.diagPos[(std::size_t)c]=
        rb+(std::int32_t)(dit-uniq.begin());
  ...
  std::printf(
    "NODALS_GPU_GATE5A_FINECSR_NEW status=PASS rows=%d nnz=%zu directed=%llu targetTotal=%llu targetMax=%u rowMax=%u rowMean=%.6f threads=%d incidenceSeconds=%.6f pass1Seconds=%.6f prefixAllocSeconds=%.6f pass2Seconds=%.6f totalSeconds=%.6f scratchMiB=%.3f csrMiB=%.3f metadataMiB=%.3f totalMiB=%.3f rssMiB=%.3f hwmMiB=%.3f\n",
    H.n,H.col.size(),
    (unsigned long long)H.directedContrib,
...
inline void gate5a_validate_finecsr(
  ...
  const bool pass=dims&&rowExact&&colExact&&diagExact&&incExact&&
                  packedExact&&contribExact&&slotExact&&statsExact;

  std::printf(
    "NODALS_GPU_GATE5A_FINECSR_PARITY status=%s dimsExact=%d rowExact=%d colExact=%d diagPosExact=%d incOffExact=%d packedExact=%d contribOffExact=%d slotExact=%d statsExact=%d refRows=%d newRows=%d refNnz=%zu newNnz=%zu refDirected=%llu newDirected=%llu refRowMax=%u newRowMax=%u refSeconds=%.6f newSeconds=%.6f speedup=%.6f compareHoldsTwoTopologies=1\n",
    pass?"PASS":"FAIL",
  ...
  if(!pass)throw std::runtime_error("Gate5A fine CSR parity failed");
}
