...
MESH="${MESH:-$HOME/Desktop/meshes/nodals_vmfl003_20D_hybrid4x_575k/foam_case/constant/polyMesh}"
PREC="${PREC:-fp32}"
...
TAG="${TAG:-VMFL003_20D_HYBRID4X_GATE5C_${PREC^^}}"
OUTDIR="${OUTDIR:-$HERE/results_rans_20d_hybrid4x}"
mkdir -p "$OUTDIR"
LOG="${LOG:-$OUTDIR/${TAG}.log}"
VTU_OUT="${VTU_OUT:-$OUTDIR/${TAG}.vtu}"

[[ -x "$EXE" ]] || { echo "ERROR: executable missing: $EXE"; exit 2; }
[[ -f "$MESH/owner" ]] || { echo "ERROR: 20D hybrid4x mesh missing: $MESH"; exit 3; }

cat <<CFG
=== GATE5C 20D HYBRID4X FP32 RANS SETTINGS ===
precision=$PREC
mesh=$MESH
vtuOut=$VTU_OUT
re=$RE
...
echo
echo "=== GATE5C 20D HYBRID4X SUMMARY ==="
...
if [[ -s "$VTU_OUT" ]]; then
  vtu_ok=1
  printf 'NODALS_GPU_RANS_GATE5C_VTU_FILE path=%s bytes=%s status=PASS\n' "$VTU_OUT" "$(stat -c %s "$VTU_OUT")"
else
  echo "NODALS_GPU_RANS_GATE5C_VTU_FILE path=$VTU_OUT status=FAIL"
fi

result_ok=0
grep -q '^NODALS_GPU_RESULT .*status=PASS' "$LOG" && result_ok=1
if (( solver_rc==0 && result_ok==1 && vtu_ok==1 )); then
  echo "NODALS_GPU_RANS_GATE5C_20D_RUNNER solverRC=0 resultPass=1 vtuPass=1 status=PASS log=$LOG vtu=$VTU_OUT"
  exit 0
fi

echo "NODALS_GPU_RANS_GATE5C_20D_RUNNER solverRC=$solver_rc resultPass=$result_ok vtuPass=$vtu_ok status=FAIL log=$LOG vtu=$VTU_OUT"
exit "$solver_rc"
