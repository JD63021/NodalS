...
TAG="${TAG:-VMFL003_20D_5SLABD_NOSUPG_GATE5H_${PREC^^}}"
OUTDIR="${OUTDIR:-$HERE/results_rans_20d_5slabD_nosupg}"
mkdir -p "$OUTDIR"
...
if (( solver_rc==0 && result_ok==1 && vtu_ok==1 )); then
  printf 'NODALS_GPU_RANS_GATE5H_VTU_FILE path=%s bytes=%s status=PASS\n' \
    "$VTU_OUT" "$(stat -c %s "$VTU_OUT")"
  echo "NODALS_GPU_RANS_GATE5H_20D_RUNNER solverRC=0 resultPass=1 vtuPass=1 status=PASS log=$LOG vtu=$VTU_OUT"
  exit 0
fi
echo "NODALS_GPU_RANS_GATE5H_20D_RUNNER solverRC=$solver_rc resultPass=$result_ok vtuPass=$vtu_ok status=FAIL log=$LOG vtu=$VTU_OUT"
exit "$solver_rc"
