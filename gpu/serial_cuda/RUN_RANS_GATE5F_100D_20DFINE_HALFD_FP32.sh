...
MESH="${MESH:-$HOME/Desktop/meshes/nodals_vmfl003_100D_20Dfine_halfD_1329k/foam_case/constant/polyMesh}"
PREC="${PREC:-fp32}"
...
# Defaults reproduce the successful 1D-long Gate5D pressure configuration.
...
SUPG_MAGIC="${SUPG_MAGIC:-9}"

TAG="${TAG:-VMFL003_100D_20DFINE_HALFD_GATE5F_${PREC^^}}"
OUTDIR="${OUTDIR:-$HERE/results_rans_100d_20dfine_halfD}"
mkdir -p "$OUTDIR"
...
cat <<CFG
=== GATE5F 100D: 20D FINE + 0.5D DOWNSTREAM RANS SETTINGS ===
precision=$PREC
mesh=$MESH
vtuOut=$VTU_OUT
axialPolicy=0-20D_original_Dover18__20-100D_halfD
re=$RE
bulk=$BULK
supg=$SUPG
...
if (( solver_rc==0 && result_ok==1 && vtu_ok==1 )); then
  printf 'NODALS_GPU_RANS_GATE5F_VTU_FILE path=%s bytes=%s status=PASS\n' \
    "$VTU_OUT" "$(stat -c %s "$VTU_OUT")"
  echo "NODALS_GPU_RANS_GATE5F_100D_RUNNER solverRC=0 resultPass=1 vtuPass=1 status=PASS log=$LOG vtu=$VTU_OUT"
  exit 0
fi
echo "NODALS_GPU_RANS_GATE5F_100D_RUNNER solverRC=$solver_rc resultPass=$result_ok vtuPass=$vtu_ok status=FAIL log=$LOG vtu=$VTU_OUT"
exit "$solver_rc"
