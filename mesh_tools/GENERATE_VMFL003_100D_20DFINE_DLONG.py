...
if len(sys.argv) != 3:
    raise SystemExit("usage: GENERATE_VMFL003_100D_20DFINE_DLONG.py source20D.msh output100D.msh")
...
L20 = 20.0 * D
N_SOURCE_SLABS = 360
N_LONG = 80             # 20D -> 100D, exactly one diameter per slab
DZ_LONG = D
EXPECTED_TOTAL_SLABS = N_SOURCE_SLABS + N_LONG
...
# Recover original plane ordering using rounded keys, but retain the actual source
# z coordinate for every one of the first 361 planes.
...
for nid,(x,y,z) in nodes.items():
    ...
    if abs(plane_z_actual[k] - z) > 1e-14:
        raise RuntimeError(f"Plane {k} has inconsistent z coordinates")
...
# Exact first-slab tet template from original mesh.
tet_templates = []
...
# Preserve 0-20D exactly from the source, then append 80 slabs of length D.
znew = list(source_z)
for i in range(1, N_LONG+1):
    znew.append(L20 + i*DZ_LONG)
...
dz_source = source_z[1]-source_z[0]
print("AXIAL_100D_20DFINE_DLONG")
print(f"  D                         = {D:.16e}")
print(f"  source_0_20D_slabs        = {N_SOURCE_SLABS}")
print(f"  source_first_dz_over_D    = {dz_source/D:.12e}")
print(f"  source_z_20D              = {znew[360]:.16e}")
print(f"  downstream_20_100D_slabs  = {N_LONG}")
print(f"  downstream_dz_over_D      = {DZ_LONG/D:.12e}")
print(f"  downstream_length_D       = 80")
print(f"  spacing_jump_at_20D       = {DZ_LONG/dz_source:.12e}")
...
print("VMFL003_100D_20DFINE_DLONG_MESH_GENERATION=PASS")
